...
- 商品详情页支持多图轮播、紧凑尺寸 pill、香型、带缩略图的雕花选择和包装选择；无香陈列不再展示前中后调。
- 购物车定位为下单前配置检查，支持收货地址选择、跨境目的地下拉、尺寸 / 香型 / 雕花 / 包装 / 贺卡修改，并保留配送与售后说明。
- 我的页使用头像资产卡和 3 x 2 功能区，保留申请批发身份、拿样包、渠道报价、素材下载、地址管理和客服售后。
...
- `cart.html`：购物车与下单前配置确认，支持选择已有收货地址和跨境目的地。
...
- 商品系列切换、系列内筛选、节日 hash、商品卡 `+` 加购、购物车角标、详情页不跳转加购、匠作 10 香型、无香说明、雕花预览、地址选择和我的页 3 x 2 功能区已检查通过。
