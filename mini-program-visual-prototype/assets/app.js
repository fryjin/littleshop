(function () {
  ...
  const catalogImages = {
    classic: [
      "assets/catalog/classic-01.png",
      "assets/catalog/classic-02.png",
      "assets/catalog/classic-03.png",
      "assets/catalog/classic-04.png",
    ],
    advanced: [
      "assets/catalog/advanced-01.png",
      "assets/catalog/advanced-02.png",
      "assets/catalog/advanced-03.png",
      "assets/catalog/advanced-04.png",
      "assets/catalog/advanced-05.png",
      "assets/catalog/advanced-06.png",
    ],
    artisan: [
      "assets/catalog/artisan-01.png",
      "assets/catalog/artisan-02.png",
      "assets/catalog/artisan-03.png",
      "assets/catalog/artisan-04.png",
      "assets/catalog/artisan-05.png",
      "assets/catalog/artisan-06.png",
    ],
    festival: [
      "assets/catalog/festival-01.png",
      "assets/catalog/festival-02.png",
      "assets/catalog/festival-03.png",
      "assets/catalog/festival-04.png",
    ],
  };
  ...
  const buildProductCards = (panel) => {
    ...
    const href = panel.dataset.cardHref || "product.html";
    const actionLabel = panel.dataset.actionLabel || (status === "零售" ? "加购" : "询价");
    ...
    for (let index = 1; index <= count; index += 1) {
      ...
      body.append(codeRow, name, meta, price);
      ...
      action.dataset.cardAction = actionLabel;
  ...
  const activateSeries = (name) => {
    seriesCards.forEach((card) => {
      const active = card.dataset.seriesCard === name;
      card.classList.toggle("active", active);
      card.setAttribute("aria-pressed", String(active));
    });
    seriesPanels.forEach((panel) => {
      panel.classList.toggle("active", panel.dataset.seriesPanel === name);
    });
  };

  seriesCards.forEach((card) => {
    card.addEventListener("click", () => activateSeries(card.dataset.seriesCard));
  });
  ...
  if (fragranceSelector) {
    const title = fragranceSelector.querySelector("[data-fragrance-title]");
    const typeText = fragranceSelector.querySelector("[data-fragrance-type-text]");
    const topText = fragranceSelector.querySelector("[data-fragrance-top-text]");
    const middleText = fragranceSelector.querySelector("[data-fragrance-middle-text]");
    const baseText = fragranceSelector.querySelector("[data-fragrance-base-text]");
    const sceneText = fragranceSelector.querySelector("[data-fragrance-scene-text]");
    const updateFragrance = (button) => {
      if (!button) return;
      if (title) title.textContent = button.dataset.fragranceName || button.textContent;
      if (typeText) typeText.textContent = button.dataset.fragranceType || "";
      if (topText) topText.textContent = button.dataset.fragranceTop || "";
      if (middleText) middleText.textContent = button.dataset.fragranceMiddle || "";
      if (baseText) baseText.textContent = button.dataset.fragranceBase || "";
      if (sceneText) sceneText.textContent = button.dataset.fragranceScene || "";
    };
    fragranceSelector.addEventListener("click", (event) => {
      const button = event.target.closest("[data-fragrance-name]");
      if (button) updateFragrance(button);
    });
    updateFragrance(fragranceSelector.querySelector("[data-fragrance-name][aria-pressed='true']"));
  }

  const packageRoot = document.querySelector("[data-package-options]");
  if (packageRoot) {
    const optionButtons = Array.from(packageRoot.querySelectorAll("[data-package-option]"));
    const delta = packageRoot.querySelector("[data-package-delta]");
    const updatePackageDelta = () => {
      const selected = optionButtons.find((button) => button.getAttribute("aria-pressed") === "true" && !button.disabled);
